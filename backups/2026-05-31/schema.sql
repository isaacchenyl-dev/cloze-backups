...
CREATE OR REPLACE FUNCTION "public"."log_word_results"("p_paragraph_id" "uuid", "p_results" "jsonb") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare uid uuid := auth.uid(); r jsonb; v_idx int; v_word text; v_outcome text;
begin
  if uid is null then return; end if;
  for r in select * from jsonb_array_elements(p_results) loop
    v_idx := (r->>'idx')::int; v_word := coalesce(r->>'word',''); v_outcome := coalesce(r->>'outcome','');
    if v_outcome not in ('correct','hint','missed') then continue; end if;
    insert into public.word_stats as ws
      (user_id, paragraph_id, token_idx, word, attempt_count, correct_count, streak, last_result, last_seen)
    values (uid, p_paragraph_id, v_idx, v_word, 1,
      case when v_outcome='correct' then 1 else 0 end,
      case when v_outcome='correct' then 1 else 0 end, v_outcome, now())
    on conflict (user_id, paragraph_id, token_idx) do update set
      attempt_count = ws.attempt_count + 1,
      correct_count = ws.correct_count + (case when v_outcome='correct' then 1 else 0 end),
      streak = case when v_outcome='correct' then ws.streak + 1 else 0 end,
      last_result = v_outcome, word = excluded.word, last_seen = now();
  end loop;
end; $$;


ALTER FUNCTION "public"."log_word_results"("p_paragraph_id" "uuid", "p_results" "jsonb") OWNER TO "postgres";
...
ALTER TABLE "public"."streak_freezes" OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."kudos"
    ADD CONSTRAINT "kudos_pkey" PRIMARY KEY ("from_user", "to_user", "week");
...
ALTER TABLE ONLY "public"."streak_freezes"
    ADD CONSTRAINT "streak_freezes_pkey" PRIMARY KEY ("user_id", "day");
...
ALTER TABLE ONLY "public"."word_stats"
    ADD CONSTRAINT "word_stats_pkey" PRIMARY KEY ("user_id", "paragraph_id", "token_idx");
...
CREATE INDEX "idx_kudos_to_week" ON "public"."kudos" USING "btree" ("to_user", "week");
...
CREATE INDEX "idx_streak_freezes_user_day" ON "public"."streak_freezes" USING "btree" ("user_id", "day" DESC);
...
CREATE INDEX "idx_word_stats_user" ON "public"."word_stats" USING "btree" ("user_id");
...
ALTER TABLE ONLY "public"."kudos"
    ADD CONSTRAINT "kudos_from_user_fkey" FOREIGN KEY ("from_user") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."kudos"
    ADD CONSTRAINT "kudos_to_user_fkey" FOREIGN KEY ("to_user") REFERENCES "auth"."users"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."streak_freezes"
    ADD CONSTRAINT "streak_freezes_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."word_stats"
    ADD CONSTRAINT "word_stats_paragraph_id_fkey" FOREIGN KEY ("paragraph_id") REFERENCES "public"."paragraphs"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."word_stats"
    ADD CONSTRAINT "word_stats_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;
...
CREATE POLICY "kudos_delete_own" ON "public"."kudos" FOR DELETE USING (("auth"."uid"() = "from_user"));



CREATE POLICY "kudos_insert_own" ON "public"."kudos" FOR INSERT WITH CHECK ((("auth"."uid"() = "from_user") AND ("from_user" <> "to_user")));



CREATE POLICY "kudos_select_involved" ON "public"."kudos" FOR SELECT USING ((("auth"."uid"() = "from_user") OR ("auth"."uid"() = "to_user")));
...
CREATE POLICY "streak_freezes_delete_own" ON "public"."streak_freezes" FOR DELETE USING (("auth"."uid"() = "user_id"));



CREATE POLICY "streak_freezes_insert_own" ON "public"."streak_freezes" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "streak_freezes_select_own" ON "public"."streak_freezes" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "word_stats_modify_own" ON "public"."word_stats" USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "word_stats_select_own" ON "public"."word_stats" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
GRANT ALL ON FUNCTION "public"."log_word_results"("p_paragraph_id" "uuid", "p_results" "jsonb") TO "authenticated";
...
GRANT REFERENCES,TRIGGER,TRUNCATE,MAINTAIN ON TABLE "public"."kudos" TO "anon";
GRANT REFERENCES,TRIGGER,TRUNCATE,MAINTAIN ON TABLE "public"."kudos" TO "authenticated";
GRANT REFERENCES,TRIGGER,TRUNCATE,MAINTAIN ON TABLE "public"."kudos" TO "service_role";
...
GRANT REFERENCES,TRIGGER,TRUNCATE,MAINTAIN ON TABLE "public"."streak_freezes" TO "anon";
GRANT REFERENCES,TRIGGER,TRUNCATE,MAINTAIN ON TABLE "public"."streak_freezes" TO "authenticated";
GRANT REFERENCES,TRIGGER,TRUNCATE,MAINTAIN ON TABLE "public"."streak_freezes" TO "service_role";
...
GRANT REFERENCES,TRIGGER,TRUNCATE,MAINTAIN ON TABLE "public"."word_stats" TO "anon";
GRANT REFERENCES,TRIGGER,TRUNCATE,MAINTAIN ON TABLE "public"."word_stats" TO "authenticated";
GRANT REFERENCES,TRIGGER,TRUNCATE,MAINTAIN ON TABLE "public"."word_stats" TO "service_role";
